angular
  ...
  .controller("form", function ($scope) {
    ...
    var passwordInput = $scope.$watchAsProperty("password");

    var passwordsEqual = passwordInput
      .combine($scope.$watchAsProperty("passwordConfirm"), function (
        pass,
        confirm
      ) {
        return {
    ...
    var usernameInput = $scope.$watchAsProperty("username");

    var passwordIsValid = passwordInput.map(longerThanOrEqual(5));

    var usernameIsFree = usernameInput
      .changes()
      .filter(longerThan(2))
      .flatMapLatest(function (it) {
        return Bacon.later(2000, it === "user");
      })
    ...
    passwordIsValid.changes().not().digest($scope, "passwordInvalid");

    passwordsEqual.changes().not().digest($scope, "passwordsDontMatch");
  });
